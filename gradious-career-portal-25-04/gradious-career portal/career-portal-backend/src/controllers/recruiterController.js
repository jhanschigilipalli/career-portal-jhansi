...
const { isRecruiterInviteExpired } = require("../utils/recruiterInviteExpiry");
const {
...
exports.validateInviteToken = async (req, res) => {
  ...
  `;
    ...
    if (isRecruiterInviteExpired(invite)) {
...
exports.completeRecruiterSignup = async (req, res) => {
  try {
    ...
    // Remove legacy duplicate row if recruiters were ever stored in `users`.
    await db.query(
      "DELETE FROM users WHERE email = ? AND LOWER(role) = 'recruiter'",
      [invitedEmail]
    );
