...
const login = async (req, res) => {
  try {
    ...
    const lookupKey = rawId.includes("@") ? rawId.toLowerCase() : rawId;

    let account = null;
    let accountType = "user";

    // Recruiters live in `recruiters` — check that table first so a legacy
    // `users` row with the same email cannot block login after invite signup.
