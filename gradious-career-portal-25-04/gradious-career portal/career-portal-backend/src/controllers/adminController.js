...
} = require("../utils/recruiterInviteExpiry");
...
exports.resendInvite = async (req, res) => {
  try {
    ...
    const newExpiry = recruiterInviteExpiresAt();
    ...
    await sendEmail({
      ...
      html: `
        ...
        <p style="margin-top:15px;">${recruiterInviteExpiryEmailNote()}</p>
      `
    });
  ...
  try {
    ...
    await db.query(
      `INSERT INTO recruiter_invites (email, token, status, expires_at)
       VALUES (?, ?, 'Pending', DATE_ADD(NOW(), INTERVAL ${RECRUITER_INVITE_TTL_MINUTES} MINUTE))`,
